...
import { android, AndroidApplication, AndroidActivityBackPressedEventData, ios } from "tns-core-modules/application";
...
class HideGestureRecognizerImpl2 extends GestureRecognizer {
    public func: () => void;

    static initWithOwner(owner) {
        const handler = new HideGestureRecognizerImpl2();
...
export class ModalComponent implements OnInit {
    ...
    onLoad({ object }) {
        ...
        // Event Propagation
        if (ios) {
            targetHandler = HideGestureRecognizerImpl2.initWithOwner(this.overlayView);
            if (this.dismissable) {
            ...
            targetHandler2 = HideGestureRecognizerImpl2.initWithOwner(this.bodyView);
