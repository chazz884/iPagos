...
class HideGestureRecognizerImpl1 extends GestureRecognizer {
    public func: () => void;

    static initWithOwner(owner) {
        const handler = new HideGestureRecognizerImpl1();
...
export class ModalComponent implements OnInit {
    ...
    onLoad({ object }) {
        ...
        // Event Propagation
        if (ios) {
            targetHandler = HideGestureRecognizerImpl1.initWithOwner(this.overlayView);
            if (this.dismissable) {
            ...
            targetHandler2 = HideGestureRecognizerImpl1.initWithOwner(this.bodyView);
